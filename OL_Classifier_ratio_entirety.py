...
tor = r'J:\博士研究\论文写作\202108第一篇小论文\dataset\2.预处理后数据\第二轮实验\2_test_tor.csv'
nor = r'J:\博士研究\论文写作\202108第一篇小论文\dataset\2.预处理后数据\第二轮实验\2_test_nor1000.csv'
modelpath = r'J:\博士研究\论文写作\202108第一篇小论文\dataset\2.预处理后数据\第二轮实验\entirety'
adaboostpath = r'J:\博士研究\论文写作\202108第一篇小论文\dataset\2.预处理后数据\第二轮实验\adaboost'

# tor = '/home/mason/MHT/202107paper/20220326/data/2_test_tor.csv'
# nor = '/home/mason/MHT/202107paper/20220326/data/2_test_nor1000.csv'
# modelpath = '/home/mason/MHT/202107paper/20220326/models'

lowbound = 1000
uppbound = 1002
increaseratio = 3

# 是否输出错误分类的流量 #已失效，这里是测试集中的错误分类样本放入训练集，这样不对。
adaboostout = 0
...
def get_tor_normal_random(tor,normal, a=1, b=2, ratio=1):
    while(a<=b):
        tor_num = len(tor)
        nor_num = math.ceil(tor_num*a)
        print('a: ', a, ' normal_num: ', nor_num)
...
def get_tor_normal(tor,normal, a=1, b=2, ratio=1):
    ...
    tor_num = len(tor)
    data = tor
    nor = normal.sample(tor_num*a)
    data = pd.concat([data, nor])
    while(a<=b):
        print('a: ', a, ' tor_num: ',tor_num,' normal_num: ', len(data)-tor_num)
        #print(data)
        yield a, data
        nor_num = tor_num*ratio
        nor = normal.sample(nor_num)
        normal = normal.drop(nor.index)
        data = nor
        a += ratio
    ...
    for i in (range(len(esmb))):
        ...
        if(label1=='normal'):
            ...
            for j in range(0, 13, 4):
                if(esmb[i:i+1][columns[j+1]].values[0]!='non'):
                    label2 = naive_esb(list(Counter(esmb[i:i+1][[columns[j+1], columns[j+2],columns[j+3],columns[j+4]]].values[0]).most_common()))
                    #label2 = slightly_esb(list(Counter(esmb[i:i+1][[columns[j+1], columns[j+2],columns[j+3],columns[j+4]]].values[0]).most_common()))
                    #label2 = strong_esb(list(Counter(esmb[i:i+1][[columns[j+1], columns[j+2],columns[j+3],columns[j+4]]].values[0]).most_common()))
                    continue
